...
func searchFormat(site, jsonStr string) []interface{} {
	...
	case "qq":
		for i := 0; i >= 0; i++ {
			shs := map[string]interface{}{}
			shs["song_id"] = safeParse(jsonStr, fmt.Sprintf("req.data.body.song.list.[%d].mid", i))
			fmt.Println(safeParse(jsonStr, fmt.Sprintf("req.data.body.song.list")))
			if shs["song_id"] == "" {
				break
			}
			shs["song_name"] = safeParse(jsonStr, fmt.Sprintf("req.data.body.song.list.[%d].title", i))
			shs["singer_id"] = safeParse(jsonStr, fmt.Sprintf("req.data.body.song.list.[%d].singer.[0].id", i))
			shs["singer_name"] = safeParse(jsonStr, fmt.Sprintf("req.data.body.song.list.[%d].singer.[0].name", i))
			shs["album_id"] = safeParse(jsonStr, fmt.Sprintf("req.data.body.song.list.[%d].album.mid", i))
			shs["album_name"] = safeParse(jsonStr, fmt.Sprintf("req.data.body.song.list.[%d].album.name", i))
			shs["source"] = site
			shs["url_id"] = shs["song_id"]
			shs["pic_id"] = shs["song_id"]
			shs["lyric_id"] = shs["song_id"]
