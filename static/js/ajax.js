    ...
    $.ajax({
        ...
        success: function (jsonData) {
            ...
            if (rem.loadPage == 1)   // 加载第一页，清空列表
            {
                ...
                musicList[0].item = [];
                addListhead(rem.searchList);      // 加载列表头
            } else {
                $(".list-foot").remove();     //已经是加载后面的页码了，删除之前的“加载更多”提示
            }

            if (jsonData.length === 0) {
                addListbar(rem.searchList, "nomore");  // 加载完了
                return false;
            }
            ...
            for (var i = 0; i < jsonData.length; i++) {
                no++;
                tempItem = {
                    id: jsonData[i].song_id,  // 音乐ID
                    name: jsonData[i].song_name,  // 音乐名字
                    artist: jsonData[i].singer_name, // 艺术家名字
                    album: jsonData[i].album_name,    // 专辑名字
                ...
                addItem(rem.searchList, no, tempItem.name, tempItem.artist, tempItem.album, true);  // 在前端显示
            }
            ...
            if (no < mkPlayer.loadcount) {
                addListbar(rem.searchList, "nomore");  // 没加载满，说明已经加载完了
            } else {
                addListbar(rem.searchList, "more");     // 还可以点击加载更多
            }
...
function ajaxDownload(music) {
    ...
    $.ajax({
        ...
        url: mkPlayer.api,
        data: {
            "types": 'download', "name": music.name, "artist": music.artist
            , "album": music.album, "url": music.url, "music": music
        },
        dataType: "json",
        success: function (jsonData) {
            layer.closeAll()
...
function ajaxPlayList(lid, id, callback) {
    ...
    $.ajax({
        type: mkPlayer.method,
        url: mkPlayer.api,
        data: {"types": "playlist", "source": "netease", "lid": lid},
        dataType: "json",
        complete: function (XMLHttpRequest, textStatus) {
        ...
        success: function (jsonData) {
            ...
            var tempList = {
                id: lid,    // 列表的网易云 id
                name: jsonData.data.name,   // 列表名字
                cover: jsonData.data.coverImgUrl,   // 列表封面
                creatorName: jsonData.data.creator_nickname,   // 列表创建者名字
                creatorAvatar: jsonData.data.creator_avatarUrl,   // 列表创建者头像
                item: []
            };

            if (jsonData.data.coverImgUrl !== '') {
                tempList.cover = jsonData.data.coverImgUrl + "?param=200y200";
            } else {
                tempList.cover = musicList[id].cover;
            }

            if (typeof jsonData.data.tracks !== undefined || jsonData.data.tracks.length !== 0) {
                // 存储歌单中的音乐信息
                for (var i = 0; i < jsonData.data.tracks.length; i++) {
                    tempList.item[i] = {
                        id: jsonData.data.tracks[i].song_id,  // 音乐ID
                        name: jsonData.data.tracks[i].song_name,  // 音乐名字
                        artist: jsonData.data.tracks[i].artist_name, // 艺术家名字
                        album: jsonData.data.tracks[i].album_name,    // 专辑名字
                        source: "netease",     // 音乐来源
                        url_id: jsonData.data.tracks[i].url_id,  // 链接ID
                        pic_id: null,  // 封面ID
                        lyric_id: jsonData.data.tracks[i].lyric_id,  // 歌词ID
                        pic: jsonData.data.tracks[i].pic,    // 专辑图片
                        url: jsonData.data.tracks[i].url   // mp3链接
                    };
                }
            }
...
function ajaxUserList(uid) {
    ...
    $.ajax({
        type: mkPlayer.method,
        url: mkPlayer.api,
        data: {"types": "userlist", "source": "netease", "offset": "0", "limit": "1001", "uid": uid},
        dataType: "json",
        complete: function (XMLHttpRequest, textStatus) {
            if (tmpLoading) layer.close(tmpLoading);    // 关闭加载中动画
        },  // complete
            ...
            if (jsonData.playlist.length === 0 || typeof (jsonData.playlist.length) === "undefined") {
                ...
                for (var i = 0; i < jsonData.playlist.length; i++) {
                    ...
                    tempList = {
                        ...
                        creatorName: jsonData.playlist[i].creator_nickname,   // 列表创建者名字
                        creatorAvatar: jsonData.playlist[i].creator_avatarUrl,   // 列表创建者头像
                        item: []
                    };
                    // 存储并显示播放列表
