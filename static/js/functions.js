...
$(function () {
    ...
    rem.wd = localStorage.getItem('keyword')
    $("#keyword").val(rem.wd)
    ...
    if (rem.isMobile) {  // 加了滚动条插件和没加滚动条插件所操作的对象是不一样的
        ...
        $("#main-list,#sheet,#search-list").mCustomScrollbar({
            theme: "minimal",
            advanced: {
        ...
        rem.searchList = $("#search-list .mCSB_container");
    }

    addListhead(rem.mainList);  // 列表头
    addListbar(rem.mainList, "loading");  // 列表加载中
    ...
    $(".round").click(function () {
        $('.btn-box .round.active').removeClass('active');
        $(this).addClass('active');
        rem.wd = $("#keyword").val()
        rem.source = $(this).data('action')
        rem.loadPage = 1
        localStorage.setItem('keyword', rem.wd)
        ajaxSearch()
    });
    ...
    $(".music-list").on("click", ".icon-play,.icon-download,.icon-share", function () {
        ...
        switch ($(this).data("function")) {
            case "play":    // 播放
                ...
                break;
            case "download":    // 下载
                ajaxUrl(musicList[rem.dislist].item[num], ajaxDownload);
                break;
            case "share":   // 分享
                // ajax 请求数据
...
function loadList(list) {
    ...
    addListhead(rem.mainList);      // 向列表中加入列表头

    if (musicList[list].item.length == 0) {
        addListbar(rem.mainList, "nodata");   // 列表中没有数据
    } else {
        ...
        for (var i = 0; i < musicList[list].item.length; i++) {
            ...
            addItem(rem.mainList, i + 1, tmpMusic.name, tmpMusic.artist, tmpMusic.album);
        ...
        if (list == 1 || list == 2) {    // 历史记录和正在播放列表允许清空
            addListbar(rem.mainList, "clear");    // 清空列表
        }
...
// 列表中新增一项
// 参数：编号、名字、歌手、专辑
function addItem(list, no, name, auth, album, remote = false) {
    var html = '<div class="list-item" data-no="' + (no - 1) + '" data-remote="' + (remote ? 1 : 0) + '">' +
...
function addListbar(list, types) {
    var html
    switch (types) {
        case "more":    // 还可以加载更多
            html = '<div class="list-item text-center list-loadmore list-clickable list-foot" title="点击加载更多数据" id="list-foot">点击加载更多...</div>';
            break;
...
function dataBox(choose) {
    $('.btn-box .tab.active').removeClass('active');
    $(".tab[data-action='" + choose + "']").addClass('active');

    $("#" + choose).fadeIn();
    ["main-list", "sheet", "search"].map(function (item) {
        if (item != choose) {
            $("#" + item).fadeOut();
        }
    });
